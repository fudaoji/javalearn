...
<head>
    ...
    <script>
        ...
        function check(event, id) {
            var cbs = document.getElementsByName("uid"), selectedTotal = 0;
            if (event.checked) {
                //3.遍历
                ...
                if (selectedTotal == parseInt("${pb.rows}")) {
                    document.getElementById("firstCb").checked = true;
                }
            } else {
                document.getElementById("firstCb").checked = false;
            }
        }
    ...
    <form id="form" action="${pageContext.request.contextPath}/delSelectedServlet" method="post">
        <table border="1" class="table table-bordered table-hover">
            ...
            <c:forEach items="${pb.list}" var="user" varStatus="s">
                <tr>
                    <td><input type="checkbox" name="uid" value="${user.id}" onclick="check(this, ${user.id})"></td>
                    <td>${s.count}</td>
                    <td>${user.name}</td>
                    <td>${user.gender}</td>
                    <td>${user.age}</td>
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <c:if test="${pb.totalCount > 0}">
                    ...
                    <li>
                        <c:if test="${pb.currentPage * pb.rows < pb.totalCount}">
                        <a href="${pageContext.request.contextPath}/findUserByPageServlet?currentPage=${pb.currentPage+1}&rows=5&name=${condition.name[0]}&address=${condition.address[0]}&email=${condition.email[0]}"
                           aria-label="Previous">
                            </c:if>
                            <c:if test="${pb.currentPage * pb.rows >= pb.totalCount}">
                            <a disabled aria-label="Next">
                                </c:if>
